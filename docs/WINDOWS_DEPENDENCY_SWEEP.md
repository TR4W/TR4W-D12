...
## Where it stands (2026-09-08, end of day)

**45 files down to 9.** Re-measure rather than trusting this line -- the
command is in [The method](#the-method-which-is-the-part-worth-copying) and it
takes seconds. What is left, and why:

| Unit                                   | Why it is still here                                                                             |
| -------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `MainUnit`, `uProgramMain`, `tr4w.lpr` | the biggest remaining bodies of Win32; not started                                               |
| `logdvp`                               | raw Win32 throughout, and its own sweep -- the note in the unit says so                          |
| `uNet`                                 | `SetEvent` and `SetSystemTime`, both real; reasons written at the import                         |
| `logsubs2`                             | the message-pump wait and `RemoveFontResourceW`; the whole bill is at the import                 |
| `uCallsigns`                           | `CompareStringA` -- one of three drifted copies, and NY4I asked for the tests BEFORE the repoint |
| `DLPortIO`                             | blocked on NY4I's LPT decision, in `BENCH_QUEUE.md`                                              |

**NOT counted, deliberately:** `tr4w/test/uTestMain.pas`. It is part of the
Radio Factory Tester, which **does not compile** and is in no build script --
see `BENCH_QUEUE.md`. An ungated import in a file no compiler reads is not a
portability problem, and counting it would make this number a fiction.

**What the day actually cost, and what it bought.** Very little of this was
translation. The conversions turned up a locale bug (`CharLowerA` folding by
the machine's codepage), four unbounded `lstrcatA` calls, a `MoveMemory` whose
arguments reverse under `Move`, a short-read bug in the band map loader, an
error message showing an unrelated OS error, a sticky-keys restore that ran
when its save had failed, and a `ForgetPanel` that cleared the whole cache
