...
the unit builds for `x86_64-linux`, which is the only question that matters and
the only one a text search cannot answer -- a scan for `Windows.` structurally
...
Very little of this was translation. The conversions turned up a locale bug
(`CharLowerA` folding by the machine's codepage), four unbounded `lstrcatA`
calls, a `MoveMemory` whose arguments reverse under `Move`, a short-read bug in
the band map loader, an error message showing an unrelated OS error, a
sticky-keys restore that ran when its save had failed, a `ForgetPanel` that
cleared the whole cache every time, and a `\Device\Beep` path that had almost
certainly done nothing on Windows 10 for years.

**And two that only a second target could expose**, both latent 64-bit defects
sitting in a green build:

- **`THandle` MEANT TWO DIFFERENT TYPES depending on uses-clause ORDER.**
  `LCLType` redeclares `THandle = type PtrUInt`, carrying Lazarus's own note
  *"Use TLCLHandle instead of this redefined THandle"*. Any unit naming LCLType
  after SysUtils got that one. Identical width on 32-bit Windows; different on
  every 64-bit target. Fixed by naming the concept --
  `utils_file.TFileHandle` for files, `VC.TLPTBaseAddress` for port addresses.
- **LPT base addresses were declared `THandle` at eight sites.** `$378` is an
  x86 I/O port address; it is never opened and never returned by the OS. The
  sentinel `THandle(-1)` would have silently widened at 64-bit, so
  `if Base = feInvalidHandle` would stop matching and TR4W would drive a port
  it believed closed.

**The portability work is a reading exercise, and the defects are what the
reading finds.** A compiler for a second target is the reading assistant.
...
| `lstrcatA` / `lstrlenA`                               | `utils_text.SetCharBufferBytes` / `Length`                                | and BOUND it -- `lstrcatA` walks to the NUL and keeps writing                                                                                            |
...
| ~~CALLS~~ | ~~`DLPortIO.pas`~~ **RESOLVED -- DELETED 2026-09-08 -- superseded, unreferenced**  | ~~`GetLastError` `lstrcat` `LoadLibrary` `GetProcAddress` `FreeLibrary`~~                                                                                                                                                                                                                                                  | ~~`DWORD` `THandle` `MAX_PATH` `BOOL`~~                                                                       |
...
| ~~CALLS~~ | ~~`logdvp.pas`~~ **RESOLVED -- compiles for Linux**                                | ~~`GetLastError` `Sleep` `lstrlen` `CreateFile` `ReadFile` `CloseHandle` `WaitForSingleObject` `timeSetEvent` `sndPlaySound`~~                                                                                                                                                                                             | ~~`HANDLE` `THandle` `BOOL` `INVALID_HANDLE_VALUE`~~                                                          |
...
| ~~CALLS~~ | ~~`logsubs2.pas`~~ **RESOLVED -- compiles for Linux**                              | ~~`GetTickCount` `Sleep` `wsprintf` `CloseHandle` `WSA`~~                                                                                                                                                                                                                                                                  | ~~`DWORD` `HANDLE` `THandle` `BOOL` `SW_` `IDNO` `SYSTEMTIME`~~                                               |
| ~~CALLS~~ | ~~`logwind.pas`~~                                                                  | ~~`GetTickCount` `wsprintf` `ReadFile` `CloseHandle` `SystemTimeToTz` `GetSystemTime`~~                                                                                                                                                                                                                                    | ~~`THandle` `BOOL` `SW_` `SYSTEMTIME`~~                                                                       |
| ~~CALLS~~ | ~~`MainUnit.pas`~~ **RESOLVED -- compiles for Linux; PINNED in Lint-LinuxCompile** | ~~`GetTickCount` `GetLastError` `Sleep` `lstrcat` `wsprintf` `CloseHandle` `FindFirstFile` `SetEvent` `SetThreadPriority` `LoadLibrary` `GetProcAddress` `FreeLibrary` `ShowWindow` `MoveWindow` `SendMessage` `GetKeyState` `SetFocus` `GetSystemMetrics` `socket` `sendto` `GetSystemTime` `sndPlaySound` `MessageBox`~~ | ~~`DWORD` `WPARAM` `HANDLE` `THandle` `BOOL` `SW_` `VK_` `IDYES` `IDNO` `INVALID_HANDLE_VALUE` `SYSTEMTIME`~~ |
...
| ~~CALLS~~ | ~~`uCallsigns.pas`~~ **RESOLVED -- compiles for Linux**                            | ~~`wsprintf` `CompareString`~~                                                                                                                                                                                                                                                                                             | ~~`BOOL`~~                                                                                                    |
...
| ~~CALLS~~ | ~~`uNet.pas`~~ **RESOLVED -- compiles for Linux**                                  | ~~`GetTickCount` `Sleep` `wsprintf` `CreateFile` `WaitForSingleObject` `SetEvent` `SendMessage` `sendto` `GetCurrentThreadId`~~                                                                                                                                                                                            | ~~`BOOL` `INVALID_HANDLE_VALUE`~~                                                                             |
