...
| `CfgCmd.pas` | `ID`, `CMD` | handed to `CheckCommand` as garbage ShortStrings; a garbage `ID` could return False, and `LogCfg.pas:634` turns that into a **modal** "invalid statement in config file" | **the line is parsed now.** It never was: nothing split `FileString`, so a config file run with ctrl-V or EXECUTE silently did nothing. Split with `Pos`/`Copy` on the ShortString, reproducing `EnmuCFGFile`'s semantics exactly — including that a line with no `=` yields an empty `ID` and is accepted-and-ignored rather than raising the modal |
