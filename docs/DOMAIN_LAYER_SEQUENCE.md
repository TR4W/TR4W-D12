...
| **2. SQLite** | ~~STARTED 2026-09-01~~ **SQLITE IS THE LOG (2026-09-08).** `uLogSource` reads the database and nothing else -- its source selection was removed on 2026-09-24 -- and the binary `.TRW` WRITE path is deleted, so the file is import-only. Order and exit criteria: [`SQLITE_MIGRATION_TASKS.md`](SQLITE_MIGRATION_TASKS.md). The DDL in [`SQLITE_LOG_SCHEMA_PLAN.md`](SQLITE_LOG_SCHEMA_PLAN.md) §4 has diverged from `src/domain/uLogSchema.pas`, which is the schema |
