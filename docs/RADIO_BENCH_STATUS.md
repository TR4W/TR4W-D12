...
| Elecraft K4 | K4 | 2026-07-31 | NY4I | Split: FT1; alone is sufficient (RX always VFO A). CW-by-CAT in use. SERIAL: RIT/XIT indicators and offset were blank (ParseIFCommand wrote only the legacy scalars, not the per-VFO copy the window reads) -- fixed and retested OK. CW by serial port (COM6, K4 serial 2) verified via the CPU keyer. NETWORK retested same day and unaffected, confirming the AI push path (RT/XT/RO) writes the per-VFO fields itself. **2026-09-20, FIRST RADIO EVER CONNECTED FROM LINUX.** NY4I ran the 5.0.12 **AppImage on Linux Mint 22.3** -- a machine that did not build it, with no TR4W dependencies installed -- reached the UI and connected to the K4 **over Ethernet direct**. Two things are proven at once and neither had been before: the gtk2 UI works from the bundled libraries on a foreign machine, and the network radio path works off Windows. **What it does NOT prove, and do not let it be quoted as if it did:** no QSO was logged, **CW cannot key there** (element timing off Windows is a placeholder `Sleep` -- `docs/PLATFORM_CLOCK_ABSTRACTION.md` part 2), serial ports are not yet selectable on Unix (`docs/PORT_IDENTITY_PLAN.md` step 5), and split/RIT/CW-by-CAT were not exercised in this session. The AppImage needed `chmod +x` first -- an HTTP download carries no executable bit, which is normal for every AppImage and is why the release note leads with it. |
| Elecraft KX3 | KX3 | - | - |  |
...
| Icom IC-775 | IC775 | - | - |  |
| Icom IC-7760 | IC7760 | 2026-09-24 | NY4I | **FIRST ICOM LAN RADIO EVER RUN AGAINST TR4W**, and it found a defect that had nothing to do with Icom. **Evidence: a 663-second capture (`pcap/ICOM-7760-10-minute-soak.pcapng`, 60,719 packets) plus a DEBUG+TRACE log.** Symptom: thirteen CI-V outages, every one exactly 12.3 s, on a repeating ~103 s cycle, while the radio kept pinging both sockets at ~100 ms throughout -- demonstrably alive, and ignoring 133 CI-V commands sent into one stall. **Root cause: NOT ONE OF THE TRANSPORT'S SIX TIMERS HAD EVER FIRED.** They were LCL `TTimer`s created on an Indy UDP reader thread, and the LCL's Win32 `TTimer` is `SetTimer(0, 0, ...)` -- `WM_TIMER` goes to the calling thread's queue, and a reader thread has no message pump. Measured consequence: 0 self-initiated pings (our ping count equalled the radio's exactly, so we only ever replied), 0 idle keepalives (the radio sent us 6,295), **0 token renewals** against a 60 s interval. The radio expires a session **~90.6 s after login** (thirteen events, sigma ~0.25 s) and stops answering CI-V; our 12.3 s recovery meant the *previous* session's expiry killed the next one, which is where the 103 s cycle comes from. **PROBE VERIFIED 2026-09-24 (NY4I, 6 m):** renewal driven off the radio's inbound pings gave renewals at session age 360 s / 420 s / 480 s on **one unchanged token** (`$182A447A`), `handshake stuck` 0, `CI-V data timeout` 0 -- the first Icom LAN session in this program's history past 91 seconds. A token changes only at login, so the constant token is the proof no new session was made. **THE SHIPPED FIX IS NOT THE PROBE AND IS NOT YET SOAKED.** The probe is removed; the six timers are now one owned timer thread (`TIcomTimerThread`), so five handlers that had never executed -- ping keepalive + dead-link detection, idle, CI-V watchdog, AYT retry, login retry -- run for the first time. **Re-soak owed**, and the thing to watch is the CI-V watchdog: `CI-V data timeout (NNNN ms), sending CivOpen` during normal operation would mean its 2 s threshold is too tight now that it actually runs. Also settled by this capture: the radio reports `CommonCap=$8010` (MAC path, so the unassigned-`FGUID` defect is real but unreachable here), it sent **no 18-byte packets at all** (the "Icom pads control packets to 18" comment is false for this model), and there were **zero** retransmit requests and **zero** inbound sequence gaps. Detail: `docs/ICOM_LAN_CROSS_CHECK_RIGPLANE.md`. |
| Icom IC-78 | IC78 | - | - |  |
| Icom IC-7800 | IC7800 | - | - |  |
| Icom IC-781 | IC781 | - | - |  |
| Icom IC-7850 | IC7850 | - | - |  |
