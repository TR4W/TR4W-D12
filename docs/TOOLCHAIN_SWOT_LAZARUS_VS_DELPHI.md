...
That question has a clean answer, and it is not close. Read on, but the short version is that the
platform matrix NY4I named cannot be reached on first-party Delphi at all.
...
### Weaknesses
- **There is no first-party Delphi Linux desktop GUI.** Delphi targets Linux, but as a *server-side*
  target; FMX's desktop story is Windows and macOS. Linux desktop requires a third-party product
  (FmxLinux). For a stated goal of Windows + Linux + macOS parity, this is a structural gap, not a
  rough edge. **[VERIFY against current RAD Studio docs before acting — this is the single most
...
**Why it does not carry, on the stated constraints:** it answers a different question. It is correct
if the platform target is Windows-only. Against Windows + Linux + macOS it fails, because *no amount
of finishing D12 produces a Linux desktop build* — that is a vendor strategy gap, not a work item.
And against "open to community contributors" it fails again, because Community Edition's
restrictions are not something the project can engineer around.
...
The reasoning in one paragraph: the expensive work — removing Win32, eradicating asm, separating
core from UI — is already committed to and is identical under both toolchains. What differs is only
the destination. Delphi cannot reach the stated destination (no first-party Linux desktop) and
cannot admit the stated contributors (Community Edition restrictions). Lazarus reaches both, is
free permanently, suits this code's dialect better, and offers a UI framework closer to what TR4W
already is. The cost of switching is at its historic minimum today and rises with every FMX form.
...
1. Install FPC/Lazarus (current stable). Record the version here.
2. Compile the test project's dependency set, `{$MODE DELPHI}`, no LCL, Indy units excluded.
3. Record every failure by category: anonymous methods, generics, RTL units, `ShortString`
   behaviour, `AnsiString`/codepage semantics, `asm`.
4. Run the tests. **A pass/fail count is the answer**; do not settle for "it compiled".
5. Repeat on Linux, then macOS. Diff the pass/fail sets between platforms — a test that passes on
   Windows and fails on Linux is a portability defect with its own address, which is the most
   valuable output the spike can produce.
...
|---|---|---|
| 1 | Does **current stable FPC support `reference to`** / anonymous methods? | The #1 technical risk. Settled by the §8 spike. |
| 2 | **Delphi's current Linux desktop GUI story** — first-party FMX Linux, or still FmxLinux? | The central argument against Delphi. If this has changed, re-weigh. |
| 3 | **Current Community Edition terms** — revenue cap, command-line compiler, renewal. | The contributor argument. |
| 4 | **Vendored Indy 10.6.3.3 under FPC** — or the cost of moving to Synapse/lNet. | 19 units. |
| 5 | **hamlib, OpenSSL** on Linux/macOS. | Expected fine — both are natively cross-platform, which is a quiet point in favour. |
| 6 | **LPT keying (`inpout32.dll`, `DLPortIO.pas`)**. | Windows-only and legacy. Likely a documented Windows-only feature rather than a port target. |
| 7 | **macOS code signing / notarisation** burden. | Applies under either toolchain; a real cost nobody enjoys. |
