...
- Are the Mac and Linux versions native?
  Yes -- and as of 2026-09-08 **both** build. The FreePascal/Lazarus migration is done, that hurdle is behind us, and the application, the server and a distributable artifact all build natively on **x86_64 Linux** (a tarball) and on **macOS / Apple Silicon** (a `TR4W.app` bundle). Almost every unit test passes on Linux; the macOS suite links and runs and stops partway on an access violation in the log repository, which is the next thing to chase. **Nobody has run the GUI on either yet** -- building is not running. **ARM is untested.** Running this on Mac and Linux is why this port even exists. See [Building on Linux](#building-on-linux) and [Building on macOS](#building-on-macos), and ["Where this actually stands"](#where-this-actually-stands) for the honest, current status -- that section is maintained; this answer is not.
