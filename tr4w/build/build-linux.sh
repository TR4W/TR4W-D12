#!/bin/sh
#
# The Linux counterpart of tr4w/FullBuild.ps1 -- run ON Linux, with FPC and
# Lazarus installed natively.
#
#   sh tr4w/build/build-linux.sh              every stage
#   sh tr4w/build/build-linux.sh --app        one stage (--tests --server --package)
#   sh tr4w/build/build-linux.sh --list       what the stages are, and stop
#
...
# The widgetset picks which interfaces.ppu is linked.  gtk2 is what the Debian
# and Ubuntu lazarus packages build; qt5 exists on some installs.  Overridable
# because getting it wrong presents as "Can't find unit Interfaces", which reads
# as a missing Lazarus rather than a missing widgetset.
...
# ---------------------------------------------------------------------------
VERDICTS="${TMPDIR:-/tmp}/tr4w-linux-verdicts.$$"
: > "$VERDICTS"
FAILURES=0
...
find_toolchain() {
   ...
   if [ -n "${FPC_HOME:-}" ]; then
      ...
      for c in "$(command -v fpc 2>/dev/null || true)" /usr/bin/fpc /usr/local/bin/fpc; do
         [ -z "$c" ] && continue
         looked "FPC: $c"
...
stage_tests() {
   phase 'Unit tests'
   search_paths Tests
   TEST_EXE="$TEST_DIR/tr4w_unit_tests_linux"
