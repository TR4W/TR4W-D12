...
begin
  ...
  if ActiveMode = Digital then
     ...
     begin
     if Settings.Dvk.Enable and Settings.Message.Enable and not BeSilent then
        begin
...
begin
  ...
  if ActiveMode in [CW, Digital] then //wli issue 276
     ...
     if Settings.Qtc.Enable then
        ...
        // else
        // if Settings.Message.Enable and not BeSilent then
        // SendCrypticMessage(UTF8Encode(CallWindowString + ' '
        //                    + Settings.Messages.QsoBeforeCw));
...
begin
  ...
  PeviousDVPEnable := Settings.Dvk.Enable;
  PreviousBeSilent := BeSilent;

  Config.CWEnable := False;
  Settings.Dvk.Enable := False;
  BeSilent := True;

  ProcessReturn;

  Config.CWEnable := PeviousCWEnable;
  Settings.Dvk.Enable := PeviousDVPEnable;
