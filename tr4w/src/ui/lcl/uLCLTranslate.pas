...

{
  ...
  HOW A LANGUAGE SUPPLIES STRINGS.  Assign LCLTranslateLookup once at startup.
...
type
   ...
   TLCLTranslateLookup = function(const aKey: string): string;

var
   LCLTranslateLookup: TLCLTranslateLookup = nil;
...
begin
   ...
   translated := LCLTranslateLookup(key);
   if translated = '' then
      begin
...
var
   i: integer;
begin
   if (aRoot = nil) or (not Assigned(LCLTranslateLookup)) then
      begin
      Exit;
      end;
