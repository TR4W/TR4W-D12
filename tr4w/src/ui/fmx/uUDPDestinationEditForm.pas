...
begin
   ...
   chkStreamAppInfo.Text := chkStreamAppInfo.Text + TC_UDPEDIT_NOTIMPLEMENTED;
...
begin
   ...
   for st := Low(TUDPStream) to High(TUDPStream) do
      ...
      if (box <> nil) and box.IsChecked then
         begin
         Include(Result, st);
         end;
      end;
...
begin
   ...
   for st := Low(TUDPStream) to High(TUDPStream) do
      ...
      if box <> nil then
         begin
         box.IsChecked := FDestination.Carries(st);
         end;
      end;
end;
...
begin
   ...
   if UDPBroadcaster.TestDestination(Trim(edtAddress.Text), TypedPort, err) then
      ...
      lblTestResult.Text := Format(TC_UDPEDIT_TESTSENT,
                                   [Trim(edtAddress.Text), TypedPort]);
      end
   else
      begin
      lblTestResult.Text := err;
      end;
end;
