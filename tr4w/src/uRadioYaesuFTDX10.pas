...
{
  Yaesu serial CAT -- ASCII/text family base (legacy rtYaesu4) for the newer
  Yaesu radios that speak ';'-terminated ASCII CAT: FTDX10 (this batch), and
  later FTDX101 / FT710.  Its own family base -- NOT shared with Kenwood or
  Elecraft even though the dialect rhymes (per NY4I: similar command sets stay
  separate items).  A per-model subclass (TFTDX10Radio) sets radioModel +
  capabilities and overrides only genuine per-model deviations.

  PORTED from the bench-proven legacy path (uRadioPolling.pFTDX10_FTDX101 +
  LOGRADIO rtYaesu4 send arm).  Differences from the Kenwood/Elecraft ASCII
  dialect that this base bakes in:
    - VFO B is read with a SEPARATE command OI; (opposite-VFO info), not a "$"
      suffix -- the FTDX10 cannot return both VFOs from one IF;.
  ...
  Poll, don't push: PollRadioState queries IF;OI;FT;TX; each cycle at a fixed
  ...
  BENCH NOTES (flag for validation on real FTDX10 hardware):
    - The full mode-char map at IF pos 22 is the common Yaesu set; verify the
      less-common chars (DATA/PSK variants) on the radio.
    - The clarifier (RIT offset) field layout at pos 15-19 is parsed sign + 4
      digits; confirm the FTDX10 returns "+0000"/"-0000" there.
}
