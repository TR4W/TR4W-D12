...
//  uStack,
  ...
  uCallsigns,
  //  uMMTTY,
  //uEditQSO,  // Issue 165 NY4I Removed this from here as there was a circular reference. It did not appear to be needed.
  uTelnet,
  TF,
  Version,
  VC,
...
procedure SendFullLogToHamScore;  // Issue #783 -- enqueue every QSO for RTC resync
procedure LogEditedContactToUDP(RXData: ContestExchange); // ny4i
procedure LookupInfoToUDP(RXData: ContestExchange); // Issue 612 ny4i
function BuildUDPContact(RXData: ContestExchange; whichUDP: UDPType): string; // Issue 612 ny4i
...
{ Handles OTRSP=RX1, OTRSP=RX2, OTRSP=RXA, OTRSP=RXI, OTRSP=STEREO function key commands }
procedure OTRSPCommand;
var
   cmd: string;
begin
   if not ycccActive then
      begin
      Exit;
      end;
   cmd := UpperCase(scFileName);
   if cmd = 'RX1' then
      begin
      YCCCSetRxMode(False, False);
      end
   else if cmd = 'RX2' then
      begin
      YCCCSetRxMode(True, False);
      end
   else if cmd = 'STEREO' then
      begin
      YCCCSetRxMode(False, True);
      end
   else if cmd = 'RXA' then
      begin
      { Follow active (TX) radio }
      YCCCSetRxMode(ActiveRadio = RadioTwo, False);
      end
   else if cmd = 'RXI' then
      begin
      { Follow inactive radio - opposite of TX }
      YCCCSetRxMode(ActiveRadio = RadioOne, False);
      end;
end;
...
begin
  (* THE NEWEST QSO, FROM THE LOG -- whichever store that is. The seek to
     -1 record from FILE_END that stood here was the binary spelling of
     "the last one". *)
  if not LogSourceOpen then Exit;
  if
    (not LogSourceReadFromEnd(1, TempRXData)) or
    (TempRXData.ceRecordKind <> rkQSO) or
    (TempRXData.ceQSO_Deleted <> tLastLogEntryIsDeleted)
    then
     begin
     LogSourceClose;
     Exit;
  ...
  if tLastLogEntryIsDeleted then
     ...
     if Assigned(externalLogger) then
        begin
        externalLogger.LogQSO(TempRXData);
        end;
     // Issue #920 -- mirror restoration to HamScore as a (re-)contactinfo
     // so the server sees the QSO come back with the same ID.
     HamScoreOnLog(TempRXData);
     end
  else
     begin
     TempRXData.ceQSO_Deleted := True;
     p := TC_YOUHAVEDELETEDTHELASTLOGENTRY;
     SendDeletedContactToUDP(TempRXData);
     if Assigned(externalLogger) then
        begin
        externalLogger.DeleteQSO(TempRXData);
        end;
     // Issue #920 -- mirror the deletion to HamScore via <contactdelete>.
     // Previously only LogContact had a HamScore hook; ALT-Y deletes
     // dropped silently from the server's view.
     HamScoreOnDelete(TempRXData);
     end;
  InvertBoolean(tLastLogEntryIsDeleted);
  (* Rewrites the newest row. The seek-and-write it replaces addressed the same
     record as "the last SizeOf(ContestExchange) bytes of the file". *)
  LogStoreUpdateNewestQSO(TempRXData);
...
{$IFDEF WINDOWS}
      (* MsgWaitForMultipleObjects woke us for a MESSAGE rather than the thread,
        and this peek is what lets the sender through so it can finish and the
...
{$IF not tDebugMode }
  if Ask then
    if YesOrNo(TC_DOYOUREALLYWANTTOEXITTHEPROGRAM) = IDno then Exit;
...
{$IFEND}
  // STEP 0, BEFORE ANYTHING ELSE: STOP TRANSMITTING.
  //
  // NY4I exited while WSJT-X was transmitting over TCI and the K3 STAYED
  // KEYED -- twice.  The first fix put this in tr4w_ShutDown, which runs
  // AFTER ExitProgram has already disconnected the radios (Step 3 below), so
  // the RX; met a closed port and was dropped SILENTLY -- while still logging
  // "returning radio 1 to receive", which is worse than not trying.
  //
  // The ports are open here and nothing has been torn down, so this is the
  // only place it can work.  Everything about it is unconditional: it does
  // not ask who keyed the radio, whether we believe it is transmitting, or
  // whether PTT VIA COMMANDS is set -- "we thought it was not transmitting"
  // is exactly how a rig gets left on, and the operator has already walked
  ...
  // Use MsgWaitForMultipleObjects to pump messages while waiting — polling
  // threads call SetDlgItemText which uses SendMessage to the main thread.
  // Plain WaitForSingleObject deadlocks because SendMessage blocks until
  // the owning thread processes the message.
  ...
  if Radio1.tFactoryObject <> nil then
     begin
     Radio1.tFactoryObject.Disconnect;
     end;
  ...
  { And the start-up panadapter retry, for the same reason: it can open a
    window, and nothing should be opening windows during teardown. }
  ...
  if UDPBroadcaster.Enabled(usScore) then
     begin
     SendScoreToUDP;
     end;
...
var
  ...
  TempMode                              : ModeType;
  perfTimeStart, perfTimeStop: TDateTime;
begin
  logger.trace('>>>Entering LogContact');

  { ONE LINE PER QSO WRITTEN, and the county-line drain comes through here
    too -- DrainPendingMultiQSORefs calls LogContact once per queued county.
    That makes "one exchange, several QSOs" observable to the UI harness,
    which had no way to see it: the corpus reads finished files and the unit
    tests cannot reach this unit at all. }

  if logger.IsDebugEnabled then
     begin
     logger.Debug('[LogContact] QSO call=%s qth=%s rst=%d nr=%d points=%d',
                  [RXData.Callsign, RXData.QTHString, RXData.RSTReceived,
                   RXData.NumberReceived, RXData.QSOPoints]);
     end;
...
//      end;
  ...
  RXData.ceQSOID2 := Cardinal(GetTickCount64);
     ...
     (* THE PER-QSO RECORDING HOOK IS GONE (2026-09-07) with the built-in MP3
       ...
       ContestExchange, which is the on-disk log record, and it is persisted as
       mp3_recorded in the SQLite log and migrated forward from v1.5 and v1.6
       logs. Removing the field would change the log format; nothing writes it
       now, and old logs keep whatever they recorded. *)
     if not SendRecordToServer(NET_QSOINFO_ID, RXData) then
        begin
        inc(tUSQ);
        end;
     perfTimeStart := Now;
     if UDPBroadcaster.Enabled(usContact) then
        begin
     ...
     // Issue #783 -- HamScore RTC realtime upload (cheap no-op when disabled).
     HamScoreOnLog(RXData);
     perfTimeStop := Now;
     ...
     if UDPBroadcaster.BroadcastAllQSOs then
        begin
        LogContactToUDP(RXData);
        end;
     // Issue #783 -- network-received QSOs also feed RTC if multi-op uplink is enabled.
     HamScoreOnLog(RXData);
     end;
  ...
  if SingleBand <> AllBands then
     begin
     if RXData.Band = SingleBand then
        begin
  ...
  //  if (DDXState <> Off) and Debug and (Config.CWTone = 0) then if Random(100) = 0 then BandUp;
...
//  if DDXState = SAndPExchangeSent then    DDX(NormalContactComplete);
  repeat
    //      PutUpCQMenu;
    //      if ReminderPostedCount = 0 then
    QuickDisplay(TC_ENTERCALLSIGNSPACEDUPECHECKALTHHEL);
//    DDX(MaybeSendANewCall);
{
    ...
    if KeyPressedMemory = ' ' then
      ...
      if ActiveWindow = CallWindow then
         begin
         // Issue #997: was asm nop (no-op then-branch)
         end
//        WindowEditor(CallWindowString, Key, ExtendedKey)
        ...
        if (Key = StartSendingNowKey) and (ActiveWindow = CallWindow) and (length(CallWindowString) >= 1) then
         ...
         if Config.AutoCallTerminate then
            ...
            begin
            QuickDisplay(TC_CONTINUEENTERINGCHARACTERSPRESSRET);
            end;
...
begin
  ...
  //   if ReminderPostedCount = 0 then
  QuickDisplay(TC_ENTEREXCHANGEPRESSENTERLOGESCAPEAB);
  if ActiveMode = CW then
     begin
     PutUpExchangeMenu;
     end;
...
begin
  ...
  (* THE QSO GOES TO THE DATABASE, AND NOWHERE ELSE -- step B5.

     The .TRW write that stood here is gone: a CreateFileA, a seek to
     FILE_END and a raw record write. The call below has been making the same
     contact in SQLite since B2, and until now the binary log was the
     authority and this was the copy. It is the other way round now, and then
     it is simply the log.

     riTotalRecordsInLog was incremented from the byte count of a successful
  ...
  (* ONE MORE RECORD, AND SHOW IT.
    ...
    tLogIndex is not touched here any more. It counted rows in a widget; the
    widget has no rows. *)
  TR4WEditableLogRefreshCount;
  TR4WEditableLogScrollToEnd;

  (* AND ANY OTHER WINDOW SHOWING THE LOG. Search is non-modal and stays open
    beside the log, so a QSO logged while it is up has to reach it -- see
    LogSearchRefreshIfOpen. *)
  LogSearchRefreshIfOpen;
  Result := True;
//  SendStationStatus;
end;
...
QSOTotals[TempBand, TempMode] is accurate for the individual bands so just sum those values to calculate the total
}
   nTotal := 0;
   // Enabled() here is an OPTIMISATION, not the rule -- Send tests it again.
   // It guards CalculateTotals, which reads the whole log.
   ...
   for TempBand := Band160 to AllBands do
      begin
      for TempMode := CW to Phone do {Both do} // Was Phone for a test do
         begin
   ...
   sBuf := sBuf + #9 + '</breakdown>' + sLineBreak +
                  ...
                  '</dynamicresults>' + sLineBreak;
   logger.debug('[SendScoreToUDP] \n %s',[sBuf]);
   UDPBroadcaster.Send(usScore, sBuf);
      end;
end;
procedure SendDeletedContactToUDP(RXData: ContestExchange);
var
...
begin
   // Enabled() here is an OPTIMISATION, not the rule -- Send tests it again.
   if UDPBroadcaster.Enabled(usContact) then  // Issue 165 NY4I
      begin
      sBuf := '<?xml version="1.0"?>' + sLineBreak +
...
function BuildUDPContact(RXData: ContestExchange; whichUDP: UDPType): string;
var
  // msg: TIdBytes;
  // sBuf: AnsiString;
   sMode : AnsiString;
   ...
   freq : integer;
   txFreq : integer;
   radioStatus: RadioStatusRecord;   // one coherent read; see where it is taken
   nMultCount: integer;
   sContestName: AnsiString;
   sOperator : AnsiString;
   sRecType: AnsiString;
   sExchange: AnsiString;
...
begin
   ...
   case whichUDP of
      ...
      udpContactInfo: sRecType := 'contactinfo';
      udpContactReplace: sRecType := 'contactreplace';
      udpLookupInfo: sRecType := 'lookupinfo';
      else
         sRecType := 'contactinfo';
      end;
   ...
   if ActiveRadioPtr.RadioModel <> NoInterfacedRadio then
      begin
      // ONE read of the radio, three fields used from it.  The split flag, the
      // RX frequency and the VFO B frequency go out together in a single UDP
      // contact record, so they have to describe the same instant: reading them
      // separately off the live record lets a QSY land in between and broadcast
      // an RX frequency from one poll with a TX frequency from another.  A
      // wrong TX frequency in a contact broadcast is not a display glitch --
      // it is what an external logger records.
      radioStatus := ReadRadioCurrentStatus(ActiveRadioPtr);
      if radioStatus.Split then
         begin
         freq := radioStatus.Freq;
         txFreq := radioStatus.VFO[VFOB].Frequency;
         end
      else
         begin
         freq := radioStatus.Freq;
   ...
   case nMultCount of
      0:
         begin
         sMultiplierFields := #9 + '<ismultiplier1>0</ismultiplier1>' + sLineBreak +
                              #9 + '<ismultiplier2>0</ismultiplier2>' + sLineBreak +
                              #9 + '<ismultiplier3>0</ismultiplier3>' + sLineBreak;
         end;
      1:
        begin
         sMultiplierFields := #9 + '<ismultiplier1>1</ismultiplier1>' + sLineBreak +
                              #9 + '<ismultiplier2>0</ismultiplier2>' + sLineBreak +
                              #9 + '<ismultiplier3>0</ismultiplier3>' + sLineBreak;
         end;
      2:
        begin
         sMultiplierFields := #9 + '<ismultiplier1>1</ismultiplier1>' + sLineBreak +
                              #9 + '<ismultiplier2>1</ismultiplier2>' + sLineBreak +
                              #9 + '<ismultiplier3>0</ismultiplier3>' + sLineBreak;
         end;
      else
         begin
         sMultiplierFields := #9 + '<ismultiplier1>1</ismultiplier1>' + sLineBreak +
                              #9 + '<ismultiplier2>1</ismultiplier2>' + sLineBreak +
                              #9 + '<ismultiplier3>1</ismultiplier3>' + sLineBreak;
         end;
      end; // case nMultCount
   ...
   Result := '<?xml version="1.0"?>' + sLineBreak +
             '<' + sRecType + '>' + sLineBreak +
             #9 + '<app>TR4W</app>' + sLineBreak +
             #9 + '<contestname>' + sContestName + '</contestname>' + sLineBreak +
   ...
   {case Contest of         // This should perhaps case off of the exchange instead of contest.
   ...
   Result := Result +
            ...
            #9 + '<zone>' + Format('%d',[RxData.Zone]) + '</zone>' + sLineBreak +
            ...
            // IsClaimedQso=0 -- same wire-format meaning as a skipped
            // QSO from the consumer's perspective.  N1MM uses the
            // same convention.
      ...
      // Note for above oldcall and oldtimestamp. See message  https://groups.io/g/N1MMLoggerPlus/message/70623 why these fields
      // existed. This was for old consumers of UDP data that could not handle the GUID field. It has been 3 years since this was done.
      // As sending the old call and old timestamp is non-trivial, this program will not send th eidfference. It assumes the consumer of the UDP
      // records has updated their software to handle the ID field.
      // I left the fields here in case there is software that marked them reequired in the XML.
      // Tom NY4I 4 May 2025
end;
...
begin
   ...
   sBuf := BuildUDPContact(RXData, udpContactInfo);
   logger.Trace('Sending UDP Broadcast data: ' + sBuf);
   UDPBroadcaster.Send(usContact, sBuf);
end;

procedure LogEditedContactToUDP(RXData: ContestExchange);
var
   sBuf: AnsiString;
begin

   sBuf := BuildUDPContact(RXData, udpContactReplace);
   logger.Trace('Sending UDP Broadcast data: ' + sBuf);
   UDPBroadcaster.Send(usContact, sBuf);
end;
...
begin
   ...
   sBuf := BuildUDPContact(RXData, udpLookupInfo);
   logger.Trace('Sending UDP Broadcast data: ' + sBuf);
   UDPBroadcaster.Send(usLookup, sBuf);
end;


// Issue #783 -- HamScore RTC: walk the binary log file and enqueue every
// good QSO via HamScoreOnLog so the server can rebuild its view from
// scratch.  Pairs with HamScoreResyncFromScratch which enqueues the
// <deletelog> separator first.  Mirrors SendFullLogToUDP's iteration
// pattern (same OpenLogFile / ReadLogFile / GoodLookingQSO trio).
procedure SendFullLogToHamScore;
var
  nCount: Integer;
begin
  nCount := 0;
  ...
  try
    LogSourceRewind;
    while LogSourceNext( TempRXData ) do  // fills global TempRXData
       begin
    ...
    LogSourceClose;
    Logger.Info('[SendFullLogToHamScore] Enqueued %d QSO(s) for RTC resync', [nCount]);
  except
    on E: Exception do
       begin
       Logger.Error('[SendFullLogToHamScore] %s', [E.Message]);
       end;
  end;
end;
...
begin
   ...
   try
      ...
      LogSourceRewind;
      while LogSourceNext( TempRXData ) do  // fills the TempRXData global
         begin
         if GoodLookingQSO then
            begin
            inc(nCount);
            ...
            //SendDeletedContactToUDP(TempRXData);
            LogEditedContactToUDP(TempRXData);
            end
         else
            begin
            if TempRXData.ceQSO_Deleted  then
               begin
               logger.trace('[SendFullLogToUDP] Contact for %s not sent to UDP as it was DELETED',[TempRXData.Callsign]);
               end
            else
               begin
               logger.error('[SendFullLogToUDP] Was not valid GoodLookingQSO %s',[TempRXData.Callsign]);
               end;
            end;
         end;
      CloseLogFile;
