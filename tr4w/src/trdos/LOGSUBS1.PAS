...
begin // 1
  ...
  if (ActiveMode = CW) and (Config.CWEnable = False) then
     begin
     goto DontCheckPTT;
     end;
  ...
  begin
    ...
    if ActiveMode = CW then
       ...
       if Config.AllCWMessagesChainable then
          ...
          // A new function-key message first stops whatever is still going out.
          //
          // This used to be skipped entirely when the active radio declared
          // rcCWFlushDisruptsTiming, which every Icom did -- ny4i Issue 145,
          // "Don't do this for Icom radios.  It messes up the times of CW
          // Messages."  The cost was measured on an IC-7100 (NY4I, 2026-08-04):
          // F9 pressed 1.05 s into an F1 message neither stopped it nor keyed
          // until it finished, because FlushCWBuffer never ran and so no StopCW
          // was ever sent.
          //
          // The skip predated the 2026-08-01 K3 fix in TCWKeyerCAT.Flush.  Until
          // then Flush fired a keyer abort before EVERY function-key message,
          // including with the radio sitting idle -- on a rate-limited CI-V send
          // queue that is exactly what would wreck message timing.  Flush now
          // aborts ONLY when CWByCAT_Sending is true, so the mechanism Issue 145
          // worked around was already gone at its source.
          //
          // BOTH HALVES BENCH-VERIFIED on the IC-7100 (NY4I, 2026-08-04):
          // interrupting works, and a plain F1 keys clean -- "the CW sent by the
          // Icom 7100 sounded 100% accurate".  rcCWFlushDisruptsTiming was
  ...
  { Does this really ever work? }
{
  if StringHas(CallWindowString, ',') then
    begin
      ...
      TailEnding := True;
      AddStringToBuffer(TailEndMessage + ' ' + TailEndCallString, Config.CWTone);
    end;
}
end;
...
begin
  ...
  if ActiveRadio = RadioOne then
     begin
     EnableElement(mweRadioOne, False);
     EnableElement(Radio1.FreqElement, False);
     EnableElement(mweRadioTwo, True);
     EnableElement(Radio2.FreqElement, True);
     ...
     begin
     EnableElement(mweRadioOne, True);
     EnableElement(Radio1.FreqElement, True);
     EnableElement(mweRadioTwo, False);
     EnableElement(Radio2.FreqElement, False);
...
//  ClearRadioStatus(ActiveRadioPtr); //n4af 4.71.3
  ...
  UpdateTotals2;
  // THE ACTIVE-RADIO TINT.  These were two InvalidateRects, which worked
  // because the panels were dialogs and repainting re-ran WM_CTLCOLORDLG.  The
  // panels are LCL forms now and the tint is a property, so the form is asked
  // to recompute it instead.
...
var
  ...
  Freq: longint;
begin
  // READ ONCE.  This used to read ActiveRadioPtr.FilteredStatus.Freq twice --
  // once to work out which band it is on, once to store it -- with the polling
  // thread free to update the radio in between.  A QSY landing in that gap
  // passes the band gate on the OLD frequency and then writes the NEW one into
  // that band's memory, so the operator's 20m memory quietly acquires a 40m
  // frequency and the next band change tunes somewhere wrong.
  //
  // Note this is not a torn read and does not need a snapshot: a single
  // aligned 32-bit read is atomic.  The defect was reading the same field
  // twice and assuming the two answers agreed.
  Freq := ActiveRadioPtr.FilteredStatus.Freq;

  CalculateBandMode(Freq, Band, Mode);
  Mode := ActiveMode;
  if (Band = ActiveBand) and (Mode in [CW..Phone]) then
     begin
     FreqMemory[ActiveBand, ActiveMode] := Freq;
     end;
...
begin
  ...
  QuickDisplay(TC_SELECTSPOTRETURNLEFTORRIGHTARROWSE);
end;
