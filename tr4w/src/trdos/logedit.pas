...
function InitialExchangeEntry(Call: CallString): string;
...
uses
  ...
  uMainForm,   { the exchange field, named -- wh[] round 3 }
...
begin
  ...
  for w := Low(TMainWindowElement) to High(TMainWindowElement) do
    if TWindows[w].mweI = 1 then
       begin
       (* By element -- a loop, see MainUnit's. *)
       SetElementText(w, '');
       end;
  {
    Windows.SetWindowTextA(w h[mweCountryName], nil);
    Windows.SetWindowTextA(w h[mweBeamHeading], nil);
    Windows.SetWindowTextA(w h[mweLocator], nil);
    Windows.SetWindowTextA(w h[mweName], nil);
    Windows.SetWindowTextA(w h[mweUserInfo], nil);
    Windows.SetWindowTextA(w h[mweLocalTime], nil);
  }
...
begin
  ...
  // ShowSpotInfo stood here.  DisplayBandMap now goes through the band map
...
{ A REFUSAL THE OPERATOR CAN SEE.
  ...
  This gate is deliberate -- a single-band contest does not change band once
  there are QSOs in the log -- but it used to be SILENT, and both the Band
  Up/Band Down menu items and Alt-B/Alt-V land here, so the key did nothing and
  said nothing.  NY4I lost a testing session to it on 2026-08-24: MULTIPLE BANDS
  was FALSE in settings\tr4w.json with three QSOs logged, and the only way to
  find that out was to read the config.

  Prefer a reported refusal to a silent one -- the same rule that fixed the
  WinKeyer that failed to open and downgraded without a word. }
procedure BandDownOrUp(Direction: DirectionType);
begin
  ...
  begin
     if CommandUseInactiveRadio then
       //KK1L: 6.73 Band change on inactive radio via command
        ...
        // Pass the radio whose band is being changed: the coverage filter in
        // BandChange must ask THAT radio what it can transmit on, not whichever
        // one happens to be active.
        BandChange(InActiveRadioPtr.BandMemory {BandMemory[InactiveRadio]},
          Direction, InActiveRadioPtr);
        end
     else
        begin
        PutRadioOutOfSplit(ActiveRadio); // n4af 4.46.5
...
begin
  Result := 0;
  if Mode = FM then Mode := Phone;   // FM shares the Phone slot (DX/Zone arrays are CW..NoMode)
...
begin
  ...
  if MultByMode then
     ...
     SetMultNeedBands(Phone, OutputValue);
     ...
     SetMultNeedBands(Both, OutputValue);
...
{ FILLS ONE REMAINING-MULTIPLIER WINDOW.

  The list holds a PACKED (type, index) PAIR and no text -- MakeLong(Ord(rmt),
  i), exactly what LB_ADDSTRING's lParam carried -- and the window resolves each
  cell to a string when it paints.  See uRemMults.ResolveMult for why that stays
  paint-time.
  ...
  still needed.  That is `if HiLight or <still needed>`, one condition, same
  set of entries.  The goto existed to jump INTO the other branch's body, which
  is the shape that makes a filter hard to read. }
    ...
    rmDomestic:
      begin
        ...
        for i := 0 to mo.DomList.Count - 1 do
           begin
           if HiLighting                                                   or
              (not mo.DomList.StringIsDupeByIndex(i, TempMultBand, MultMode)) then
              begin
              Add(i);
              end;
           end;
    ...
    rmPrefix:
      begin
        for i := 0 to mo.PrfList.Count - 1 do
           begin
           if HiLighting                                                or
              (not mo.PrfList.StringIsDupeByIndex(i, MultBand, MultMode)) then
              begin
              Add(i);
              end;
           end;
      ...
      begin
        if MaxNumberOfZones <> 0 then
           ...
           for i := 1 to MaxNumberOfZones do
              begin
              if HiLighting                            or
                 mo.IsZnMult(i, MultBand, MultMode)    then
                 begin
                 // EUHFC numbers its zones from zero; everything else from one.
    ...
    rmDX:
      begin
        ...
        for i := 0 to CTY.ctyNumberCountries - 1 do
           ...
           if HiLighting                                or
              mo.IsDXMult(i, TempMultBand, MultMode)    then
              begin
              Add(i);
              end;
           end;
...
(* THE NAME FROM A RECENT QSO -- READ FROM THE LOG, NOT FROM THE WIDGET.

  THIS WAS THE LAST PLACE THE LISTVIEW WAS THE MODEL. It asked the control for
  the text of a cell, which meant the QSO data a CW message sends existed, at
  that moment, only as characters in a Win32 widget. That is what blocks the
  editable log from becoming an LCL virtual list: a TListView's TListItems would
  be EMPTY while the control was full, so this call would return nothing and no
  compiler would say a word. (The other such read, in LastEntry, was repointed
  ...
  wants the old behaviour back wants a bug back.

  A LOG-BACKED READ CAN FAIL, and an empty Str20 is the right answer when it
  does -- the caller appends the result to a CW buffer, where '' sends nothing.
  That is exactly what the widget returned when the row was empty. *)
...
begin
  ...
  (* A RECORD COUNT, AND THE LOOP BOUND CHANGED WITH IT -- deliberately, and it
    is not an off-by-one.

    This was `GetFileSize div SizeOf(ContestExchange)`, which counts the HEADER
    as a record: it returns N + 1, so `1 to records - 1` walked 1..N and did
    reach every QSO. It was right only because SizeOfTLogHeader and
    SizeOf(ContestExchange) are BOTH 376 bytes. Nothing requires that, and one
    field added to TLogHeader would have silently dropped the oldest QSO from
    every reverse scan -- with no error, just a slightly wrong answer.

    LogSourceRecordCount returns N, so the bound becomes `1 to records`, which
    walks the same 1..N. Same records, stated rather than coincidental. *)
  records := LogSourceRecordCount;

  for Offset := 1 to records do
     begin
     if not LogSourceReadFromEnd(Offset, TempRXData) then
        begin
        goto ExitLabel;
        end;
     if InMyLog then
...
{ ONE PARTIAL MATCH INTO THE SCP WINDOW.

  Was LB_ADDSTRING followed by LB_SETITEMDATA carrying the dupe flag, into a
  list box the owner-draw then read back with LB_GETTEXT and LB_GETITEMDATA
  because it had no other way to know what it was painting.  The window keeps
  the pair itself now; MasterAddCall returns False when it is full, which is
  what the ItemsInMasterListBox / MaxItemsInMasterListBox pair was counting. }
procedure DisplaySCPCall(Call: CallString; Radio: RadioPtr {RadioType});
  {KK1L: 6.73 Added Radio to allow SO2R SCP}
begin
  if not MasterWindowOpen then
     begin
     Exit;
     end;

  MasterAddCall(string(Call),
                VisibleLog.CallIsADupe(Call, ActiveBand, ActiveMode));
end;
...
begin
 ...
 //    CallsignsList.DisplayDupeSheet(Radio.BandMemory { BandMemory[Radio]}, Radio.ModeMemory { ModeMemory[Radio]}); {KK1L: 6.73 suppory SO2R SCP}
  ...
  // ONE REPAINT FOR THE WHOLE REBUILD.  The list box repainted per LB_ADDSTRING
  // and nobody chose that; a grid drawing from a model outside itself has to be
  // told when the model moved.  Same lesson as the possible-call list.
  MasterCallsUpdated;
end;

(* EditableLog.ShowMissingMultiplierReport(h: HWND) was removed 2026-09-05 --
  it had zero callers anywhere in the tree (a full-tree case-insensitive
  search found only its own declaration and this implementation) and its
  body was raw Win32: tCreateStaticWindow (CreateWindowEx of SS_* static
  controls) laid out by hand under a caller-supplied HWND, finished with
  Windows.SetWindowTextA(h, ...). Nothing on the LCL side is affected --
  there was no live caller to repoint. If a "missing multiplier report"
  window is wanted again, it should be a designed LCL form, not a caller
  that hands this routine a raw HWND to draw into. *)
      ...
      BackSpace:
        begin
          if length(InputString) > 0 then
             begin
             InputString[0] := AnsiChar(length(InputString) - 1);
...
begin
  ...
  if (QTCNumberString = '') or (NumberMessagesString = '') then
     begin
     DoABeep(Beepsingle);
     showwarning(TC_QTCPROCESSTERMINATED);
     Exit;
     end;
  ...
  if Result1 <> 0 then
     begin
     DoABeep(Warning);
     showwarning(TC_QTCNUMBERILLEGALCHARACTERS);
     Exit;
     end;
  ...
  if Result1 <> 0 then
     begin
     DoABeep(Warning);
     showwarning(TC_NUMBERMESSAGESILLEGALCHARACTERS);
     Exit;
     end;
  ...
  if (Quantity < 1) or (Quantity > 10) then
     begin
     DoABeep(Warning);
     showwarning(TC_QTCPROCESSTERMINATEDINVALIDNUMBERM);
     Exit;
     end;
...
begin
  ...
  { THE LINE ABOVE IS THE ONLY THING THAT EVER SET TempString, and it is
    commented out -- so the test below read an uninitialised local
    ShortString. Its length byte is whatever the stack held, so the window
    filled with binary garbage, and worse: non-empty garbage made both this
    test and the "if TempString = ''" below FALSE, which skipped the
    callsigns-list lookup AND the computed exchange. The garbage then went
     ...
     case ActiveInitialExchange of
       ...
       CustomInitialExchange:
         begin
           ...
           while CustomString <> '' do
              ...
              if Command = 'USER1' then
                if CD.GetEntry(Call, data) and (data.User1 <> '') then
                   begin
                   Exchange := Exchange + data.User1 ; // hh
                   end;
           ...
           if Exchange <> '' then
              begin
              getridofpostcedingspaces(Exchange);    // 4.123.9
       ...
       ZoneInitialExchange:
         begin
           ...
           { ctyGetZone reports failure with TWO different values, and they
             ...
             (reported by NY4I against CQP, 2026-08-31).

             Note the guard needs BOTH tests, not just "> 0": DUMMYZONE is 255,
             so a "> 0" test on its own would print 255 for a call that could
             not be located.  Every other zone call site in this tree tests
             only "> 0" -- including two in the CustomInitialExchange arm of
             this same function -- so they carry that inverse gap. Left alone
             here deliberately: that is a separate defect on display paths this
             change is not about. }

           TempZone := ctyGetZone(Call);

           if (TempZone <> DUMMYZONE)  and
              (TempZone <> 0)          then
              begin
              Str(TempZone, TempString);
              end;
           GetRidOfPrecedingSpaces(TempString);

           { A DebugFlag fallback that substituted a hardcoded '40' for an
             unresolved zone used to sit here.  It was removed (NY4I,
             2026-08-31): DebugFlag is set by the DEBUG and TRACE command-line
             parameters, which are in routine use, so this injected a fake zone
             into a live operator field rather than into anything diagnostic.
             An unresolved zone now yields an empty initial exchange in every
             build, which is what the rest of the tree already does. }
 ...
 while TempString <> '' do
    ...
    if DoingDomesticMults then // Gav 4.44.8    Uncommented this section
      if not StringIsAllNumbersOrSpaces(TestString) then
         ...
         logger.debug('Calling FoundDomesticQTH from InitialExchangeEntry');
         if FoundDomesticQTH(TempExchange) then
            begin
            VisibleLog.ShowDomesticMultiplierStatus(TempExchange.DomesticQTH);
            end;
