...
function GetRandomDomesticQTH(Call: CallString): string;
...
begin
  if (Random(15) > 0) or (Config.CWTone = 0) then
     begin
...
//    (DomQTHTable.ActiveDomQTHFile = 'JAPREFCT')) then
  ...
  if CountryTable.GetCountryID(CountryTable.GetCountry(Call, True)) = 'KH6' then
     begin
     QTH := 'HAW';
     end;
  if CountryTable.GetCountryID(CountryTable.GetCountry(Call, True)) = 'KL' then
     begin
     QTH := 'AK';
     end;
     ...
     case Number of
       '1', '9':
         begin
           ...
           if Copy(Call, 1, 3) = 'VY1' then
              begin
              QTH := 'NT';
              end;
           if Copy(Call, 1, 3) = 'VO1' then
              begin
              QTH := 'NF';
              end;
         end;
  ...
  case ActiveExchange of
    ...
    RSTQSONumberExchange:
      begin
        ...
        if Random(5) = 0 then
           begin
           QSONumber := QSONumber * 10 + Random(10);
           end;
        Str(QSONumber, Exchange);

        if Random(2) = 0 then
           begin
           if QSONumber < 10 then
              begin
              Exchange := '0' + Exchange;
              end;
           if QSONumber < 100 then
              begin
              Exchange := '0' + Exchange;
              end;
           end;
        ...
        if Random(2) = 0 then
           ...
           for CharPointer := 1 to length(Exchange) do
              begin
              if Exchange[CharPointer] = '1' then
                 begin
                 Exchange[CharPointer] := 'A';
                 end;
              if Exchange[CharPointer] = '9' then
                 begin
                 Exchange[CharPointer] := 'N';
                 end;
              if Exchange[CharPointer] = '0' then
                 begin
                 Exchange[CharPointer] := 'T';
                 end;
              end;
           end;
