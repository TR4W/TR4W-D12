...
(* THE ZONE THIS CONTEST'S EXCHANGE ACTUALLY CARRIES -- step C3.
  ...
  Measured against the D7 reference on iaru_hf_2026_ny4i: D7 sent 8, the ITU
  zone; this program sent the CQ zone. An IARU entrant therefore submitted the
  wrong zone on every QSO -- not a corpus artifact, a scoring error in a real
  contest.

  THE RULE IS THE CONTEST TABLE, NOT A LIST OF CONTEST NAMES. ContestsArray
  already records which zone a contest is about, in ZnM, and that is the fact
  being asked for. Keying off the exchange SHAPE instead would have been wrong
  in both directions: CROATIAN and GAGARIN-CUP use RSTZoneOrDomesticQTH -- the
  same arm as CQ-160 -- and both are ITUZones contests, while CQ-160 is not. The
  shape and the zone kind cut across each other.

  ORDER OF PREFERENCE, and the fallback matters as much as the override:

MY ITU ZONE, when the operator set it. Issue #930 added it precisely for
countries that span several ITU zones, where no lookup can be right.

CTY.DAT's ITU zone for our own callsign, when they did not. This is what
...
procedure CreateCabrilloFile;
  ...
  SetCharBuffer( tReportsFilename,
     CharBufferText( TR4W_LOG_PATH_NAME ) + Settings.My.Call + '.LOG' );
  ...
  (* THE TRANSMITTER-1 PROMPT IS HELD SHUT DELIBERATELY (2026-09-01).
     ...
     This read
        ...
        SetTransmittersId := CabrilloTagItemIndex( ctCategoryTransmitter ) = 1;

     and it could never be true, on either side of the port: the tag was marked
     ctrList FALSE, so the row was a free-text EDIT, and CB_GETCURSEL is not a
     message an edit handles -- DefWindowProc returned 0.  (The control id was
     written $CF with a decimal comment of 124 beside it; $CF is 207, which is
     Ord(ctCategoryTransmitter) + 200, so the code was right and the comment had
     been wrong for years.)

     CATEGORY-TRANSMITTER IS AN EDITABLE COMBO NOW, so that line WOULD start
     firing the first time an operator picks TWO -- and what it fires is
     SetTransmittersIdentifiers, which asks him to type "transmiter 1 computers
     IDs" into Radio1IDs.  Radio1IDs is declared, zeroed here, assigned there,
     AND READ BY NOTHING.  So the only effect of waking it today is a prompt
     that collects a value and discards it, which is a worse experience than
     the silence it replaces.

     WHAT IT WAS REACHING FOR IS REAL AND IS STILL OWED.  Cabrillo v3's QSO line
     ends with a transmitter id: a single 0 or 1 identifying RUN/MULT or
     RUN1/RUN2, used in the one- and two-transmitter categories (M/2, CQWW M/S)
     and NOT in single-op or M/M.  TR4W does emit that column -- T4, in
     tGenerateLogPortionOfCabrilloFile -- and it derives it from the wrong
     thing:
     ...
     Radio1IDs -- ten characters, "transmiter 1 computers IDs" -- is exactly the
     missing input: the set of computer ids belonging to transmitter 1, against
     which each QSO's ceComputerID would be tested.  Finishing that is a change
     to what a submitted log contains, so it is NY4I's to schedule; holding the
     prompt shut until then costs nothing and keeps the intent on the page. *)
  SetTransmittersId := False;

  (* IOTA ASKED CONTROL 120, AND 120 WAS NEVER A DROP-DOWN.

     The dialog gave its LABELS ids Ord(tag) + 100 and its FIELDS Ord(tag) +
     200, so 120 is the CATEGORY-SOAPBOX label -- a static.  CB_GETCURSEL to a
     static returns 0, so this test could not become true and IOTA has never
     set transmitter identifiers by this route.  Left disabled rather than
     repointed at a guess: the intended tag was probably CATEGORY-TRANSMITTER,
     which the line above already reads for every contest, and making IOTA
     behave differently is a change for NY4I to want, not a port to invent.

  if Contest = IOTA then
    if <the intended tag> = 1 then
       begin
...
procedure ExportTo3830Scores;
  ...
  buf := sysutils.Format( 'SOFTWARE: %s https://tr4w.net'#13#10#13#10,
     [ TR4W_CURRENTVERSION ] );
  buf := buf + '3830Scores summary'#13#10#13#10;

  // Metadata from the Cabrillo Summary fields the operator saved, read
  // silently -- no dialog popup.  Empty values emit blank lines so the operator
  // types them directly on 3830's form.
  //
  // FROM settings\tr4w.json VIA uCabrilloHeader, not from tr4w.ini.  These
  // three were still reading the ini directly after [REPORT] moved to the JSON
  // store on 2026-08-16 -- so on any station that had edited its Cabrillo
...
procedure WriteTitleBlockToSummarySheet;
  ...
  for TempTag := Low( CabrilloTags ) to High( CabrilloTags ) do
     ...
     (* ASKED OF uCbrSum, NOT OF A WINDOW HANDLE.  This used to be
        GetDlgItemTextA( CreateCabrilloWindow, Ord( TempTag ) + 200, ... ) --
        this unit reaching into another unit's dialog by an id it computed
        itself, and returning empty whenever no dialog existed. *)
     TagValue := CabrilloTagText( TempTag );
     if TagValue = '' then
        begin
        Continue;
        end;

     (* %22s right-justifies in both wsprintf and SysUtils.Format, so the
        column is unchanged.  Copy from 2 drops the stored key's leading
        underscore, which the old code did by taking the address of the tag's
        second character. *)
     sWriteFileFromString( tReportFileWrite,
...
function CalculateTotals: boolean;
  ...
  (* THE SOURCE SEAM -- step B3.  See ExportToADIF above. *)
...
1:
  if LogSourceNext( TempRXData ) then
     ...
     (* ceQSO_Skiped was tested here as a SECOND delete flag, immediately
        after ceQSO_Deleted and doing the same thing. One flag now. *)
...
(* MakeCategoryString IS DELETED, not ported.  It had no caller, and it could
   not have worked if it had one: it read control ids 121..124, and the dialog
   numbered its LABELS Ord(tag) + 100 -- so 121..124 were past the last label
   (120, CATEGORY-SOAPBOX) and did not exist.  It returned three spaces.
   Preserved in git history, where a caller-less routine that reads absent
   controls belongs. *)
...
procedure ExportToEDIByBand( Band: BandType );
  ...
  (* THE FOUR ADDRESS TAGS, JOINED.  Was lstrcatA into wsprintfBuffer after a
     ZeroMemory; a string says the same thing and cannot overrun the buffer,
     which the concatenation of four unbounded header fields could.  Empty tags
     are skipped, so only a value contributes its trailing space -- unchanged. *)
  AddressLine := '';
  for TempTag := ctAddress to ctAddressCountry do
     begin
     if CabrilloTagText( TempTag ) <> '' then
        begin
        AddressLine := AddressLine + CabrilloTagText( TempTag ) + ' ';
        end;
     end;
  ...
  sWriteFileFromString( tReportFileWrite,
     ...
     AddressLine,
     CabrilloTagText( ctCategoryOperator ), QSOTotals[ Band, Both ] ] ) );
...
procedure ExportToADIF;
  ...
  SetLength( records, 0 );
  LogSourceRewind;
  (* INTO TempRXData, not into a local: GoodLookingQSO reads that global. The
     same coupling the binary loop had, made visible rather than removed --
     removing it belongs to B4, and doing it here would put a second change
     inside the one comparison this step exists to make. *)
  while LogSourceNext( TempRXData ) do
     begin
     if GoodLookingQSO then
        begin
...
function CabrilloLanguageTag: string;
{ ' (language: es)' when a translation is loaded, empty otherwise.

  A Cabrillo file is read by contest robots and by humans checking a log,
  so when the operator ran a translated UI it is worth saying which
  catalogue produced it -- it explains any non-English text further down
  (NY4I, 2026-08-27). English adds nothing: an absent tag is the default
  build. }
begin
   Result := '';
   if ActiveUILanguage <> '' then
      begin
      Result := ' (language: ' + ActiveUILanguage + ')';
      end;
end;
...
function tGenerateSummaryPortionOfCabrilloFile: boolean;
  var
    ...
    ControlID:
      integer;
    (* TempErmakField:
      TErmakFields; *)   { ERMAK commented out }
    TempInteger:
      integer;
      const
        op = 'OPERATORS: ';
        ...
        //
        // THE WINDOW-OR-STORE FORK MOVED TO uCbrSum.CabrilloTagText.  It used to
        // stand here, and here only, which is what made it a defect waiting to
        // happen: the interactive path read dialog controls by id and headless
        // /EXPORT read settings\tr4w.json, through two different pieces of code
        // that had already drifted once -- the ini arm honoured the buffer size
        // and the json arm did not.  One answer now, asked the same way by both.
        //
        // Why the fallback exists at all: /EXPORT never opens the window, and
        // reading a control off a zero HWND returned empty, so the LOCATION
        // guard below aborted every Winter Field Day and ARRL10 batch export in
        // silence and the header dropped NAME/CLUB/ADDRESS.
        //
        // Still a buffer here rather than a string, because every caller below
        // tests the length or TempBuffer2[0] and rewriting those is a separate
      ...
      if ErmakSpecification then
         ...
         for Operator         := 1 to 10 do
            ...
            for TempErmakField := Low( TErmakFields ) to High( TErmakFields ) do
               ...
               // explicitly: HeaderTagText has no default parameter, and the
               // loop below tests TempBuffer1[0] against '1' to decide whether
               // this operator slot is populated.
               TempInteger := HeaderTagText( ERMAKSECTION, TempBuffer2,
  ...
  // (FormatCabrilloExchange) so the exchange formatting is unit-testable.
    ...
    function tGenerateLogPortionOfCabrilloFile: boolean;
      ...
      var
        ...
        // hundred lines later by FormatCabrilloExchange.  Delphi's allocator
        // happened not to reuse the block in between, so it worked by luck.
        // FPC's does reuse it, and the Cabrillo RST column came out holding
        ...
        1:
          if LogSourceNext( TempRXData ) then
             ...
             if TempRXData.RSTSent < 25 then
                ...
                begin
                RSTSent := sysutils.Format( '%d', [ TempRXData.RSTSent ] );
                end;
             ...
             if ( TempRXData.ExtMode in [ eFT8, eMFSK, eJT65 ] ) and
                ...
                begin
                RSTReceived := sysutils.Format( '%d', [ TempRXData.RSTReceived ] );
                end;
      ...
      end;
  {
    ...
    if (
    (TempRXData.ceQSO_Deleted = False) and
  ...
  (* ---------------------------------------------------------------------------- *)
      ...
      try
        try
          ...
          for Index := 0 to slOperators.Count - 1 do
             begin
             logger.debug( '   %s = %s', [ slOperators.Names[ Index ],
                slOperators.ValueFromIndex[ Index ] ] );
             end;
    ...
    function SLOperatorsSortProc( List: TStringList;
      ...
      Result := i2 - i1;   (* most QSOs first -- DESCENDING, whatever the
                              comment here used to say *)

      (* A TOTAL ORDER, NOT A PARTIAL ONE -- and this is a defect fix, not
         tidying.

         Returning 0 for two operators with the same QSO count leaves their
         order to the sort, and FPC's is not stable. TStringList.CustomSort
         reaches TStringList.QuickSort, which picks its pivot at RANDOM:

             Pivot := L + Random(R - L); // they say random is best
                       -- rtl/objpas/classes/stringl.inc:1741, FPC 3.2.2

         TR4W calls Randomize, so the seed differs per run and the tie breaks
         differently each time. MEASURED on arrl_fd_2026_ny4i, where N4KIN has
         22 QSOs and NY4I and AF4O have 6 each: six exports of the SAME log by
         the SAME binary gave 'N4KIN, AF4O, NY4I' four times and 'N4KIN, NY4I,
         AF4O' twice. The sort's INPUT was identical every run -- same names,
         same order, same counts, confirmed from the debug log -- so the
         variation was entirely the pivot.

         That is a SUBMITTED FILE changing between identical runs. Nothing was
         wrong with the content and nothing failed; it simply was not
         reproducible, which is the kind of thing nobody notices until two
         people compare their copies of the same log.

         Alphabetical is an arbitrary tie-break, and arbitrary is fine here.
         STABLE is the requirement. *)
      if Result = 0 then
         begin
