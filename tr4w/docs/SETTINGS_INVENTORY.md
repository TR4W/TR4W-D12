...
|---|---|---|---|---|---|
| `ALERT COLOR` |  | global | `Settings.MainWindow.AlertColor` | string |  |
...
| `BACKUP LOG FILE NAME` |  | global | `Settings.Log.BackupFileName` | string |  |
| `BACKUP LOG FREQUENCY` |  | global | `Settings.Log.BackupFrequency` | TBackupLogFrequency |  |
| `BAND` |  | **CONTEST** | `Settings.Contest.Band` | string |  |
...
| `CATEGORY-ASSISTED` |  | **CONTEST** | `Settings.Contest.CategoryAssisted` | tCategoryAssisted |  |
| `CATEGORY-BAND` |  | **CONTEST** | `Settings.Contest.CategoryBand` | tCategoryBand |  |
| `CATEGORY-MODE` |  | **CONTEST** | `Settings.Contest.CategoryMode` | tCategoryMode |  |
| `CATEGORY-OPERATOR` |  | **CONTEST** | `Settings.Contest.CategoryOperator` | tCategoryOperator |  |
| `CATEGORY-OVERLAY` |  | **CONTEST** | `Settings.Contest.CategoryOverlay` | string |  |
| `CATEGORY-POWER` |  | **CONTEST** | `Settings.Contest.CategoryPower` | tCategoryPower |  |
| `CATEGORY-TRANSMITTER` |  | **CONTEST** | `Settings.Contest.CategoryTransmitter` | tCategoryTransmitter |  |
| `CHECK LOG FILE SIZE` |  | global | `Settings.Log.CheckFileSize` | boolean |  |
| `CODE SPEED` |  | global | `Settings.Cw.CodeSpeed` | TCwCodeSpeed |  |
...
| `CONTEST` |  | **CONTEST** | `Settings.Contest.ContestToken` | string |  |
| `CONTEST NAME` |  | **CONTEST** | `Settings.Contest.Name` | string |  |
| `CONTEST TITLE` |  | **CONTEST** | `Settings.Contest.Title` | string |  |
...
| `CW ENABLE` |  | global | `Settings.Cw.Enable` | boolean |  |
...
| `CW TONE` |  | global | `Settings.Cw.Tone` | TCwTone |  |
...
| `DOMESTIC MULTIPLIER` |  | **CONTEST** | `Settings.Contest.DomesticMultiplier` | string |  |
...
| `DVK PATH` |  | global | `Settings.Dvk.Path` | string |  |
| `DVK RECORDER` |  | global | `Settings.Dvk.Recorder` | string |  |
| `DX MULTIPLIER` |  | **CONTEST** | `Settings.Contest.DxMultiplier` | string |  |
...
| `EXCHANGE RECEIVED` |  | **CONTEST** | `Settings.Contest.ExchangeReceived` | string |  |
...
| `FARNSWORTH ENABLE` |  | global | `Settings.Cw.FarnsworthEnable` | boolean |  |
| `FARNSWORTH SPEED` |  | global | `Settings.Cw.FarnsworthSpeed` | TCwFarnsworthSpeed |  |
...
| `INITIAL EXCHANGE` |  | **CONTEST** | `Settings.Contest.InitialExchange` | string |  |
| `INITIAL EXCHANGE CURSOR POS` |  | **CONTEST** | `Settings.Contest.InitialExchangeCursorPos` | string |  |
| `INITIAL EXCHANGE FILENAME` |  | **CONTEST** | `Settings.Contest.InitialExchangeFilename` | string |  |
...
| `MODE` |  | **CONTEST** | `Settings.Contest.Mode` | string |  |
| `MULT BY BAND` |  | **CONTEST** | `Settings.Mult.ByBand` | boolean |  |
| `MULT BY MODE` |  | **CONTEST** | `Settings.Mult.ByMode` | boolean |  |
| `MULT REPORT MINIMUM BANDS` |  | **CONTEST** | `Settings.Contest.MultReportMinimumBands` | integer |  |
...
| `MY CONTINENT` |  | **CONTEST** | `Settings.Contest.MyContinent` | string |  |
...
| `OPERATING STANDARD EDIT KEYS` |  | global | `Settings.Operating.StandardEditKeys` | boolean |  |
...
| `PREFIX MULTIPLIER` |  | **CONTEST** | `Settings.Contest.PrefixMultiplier` | string |  |
...
| `QSL MODE` |  | **CONTEST** | `Settings.Contest.QslMode` | string |  |
...
| `QSO POINT METHOD` |  | **CONTEST** | `Settings.Contest.QsoPointMethod` | string |  |
...
| `ROTATOR PORT` |  | global | `Settings.Rotator.Port` | string |  |
...
| `SINGLE BAND SCORE` |  | **CONTEST** | `Settings.Contest.SingleBandScore` | string |  |
...
| `STEREO PIN HIGH` |  | global | `Settings.Cw.StereoPinHigh` | boolean |  |
...
| `WEIGHT` |  | global | `Settings.Cw.Weight` | double |  |
...
| `ZONE MULTIPLIER` |  | **CONTEST** | `Settings.Contest.ZoneMultiplier` | string |  |

## Settings still in the config array: none

The config array (`CFGCA`) was deleted on 2026-09-14. Every setting above
is a published property; a withdrawn command is a NAME in
`uCFG.RETIRED_COMMANDS`, accepted and ignored so an old file does not
raise "invalid statement in config file".

## The credentials

A credential is a `TSecretText` property. The settings file holds only
`<Name>Ref`, the setting's name; the value lives in the operating system's
vault (`uKeychain`), is masked in Preferences, and is never sent to a
multi-op peer -- all four from the one declaration.
...
**The Notes column is carried forward when regenerating**: the generator
reads this file first and puts each note back against its command. A note
whose row has gone is listed under "Notes whose row has gone" rather than
dropped.
