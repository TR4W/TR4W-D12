...
| `CALLSIGN UPDATE ENABLE` |  | **CONTEST** | `Settings.Contest.CallsignUpdateEnable` | boolean |  |
...
| `HAMSCORE ENABLE` |  | global | `Settings.Hamscore.Enable` | boolean |  |
...
## Settings still in the config array (152)
...
| `AUTO QSL INTERVAL` | no | ctInteger |  |  |
| `AUTO SEND CHARACTER COUNT` | no | ctInteger |  |  |
...
| `HAMSCORE PASSWORD` | no | ctPassword | keychain -- a credential, see the note below |  |
| `HAMSCORE USERNAME` | no | ctCaseSensitive | keychain -- case-sensitive, see the note below |  |
